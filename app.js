...
const aqiBreathingIndicator = document.getElementById("aqiBreathingIndicator");
...
async function renderCurrentWeather(city, weatherData, latitude, longitude) {
    ...
    // Fetch and render AQI data
    try {
        const aqiData = await fetchAQIData(latitude, longitude);
        renderAQI(aqiData.current);
    } catch (error) {
        // Set default AQI values if fetch fails
        renderAQI({ pm2_5: 25, pm10: 35 });
    }
...
function getAQIInfo(aqiValue) {
    ...
    if (aqiValue <= 50) {
        ...
        return { status: "Good", color: "#10b981", breathingSpeed: "4s" };

    } else if (aqiValue <= 100) {

        return { status: "Moderate", color: "#f59e0b", breathingSpeed: "3s" };

    } else if (aqiValue <= 150) {

        return { status: "Unhealthy for Sensitive", color: "#ef4444", breathingSpeed: "2s" };

    } else if (aqiValue <= 200) {

        return { status: "Unhealthy", color: "#ef4444", breathingSpeed: "1.5s" };

    } else if (aqiValue <= 300) {

        return { status: "Very Unhealthy", color: "#7c3aed", breathingSpeed: "1s" };

    } else {

        return { status: "Hazardous", color: "#991b1b", breathingSpeed: "0.5s" };
...
function renderAQI(aqiData) {
    ...
    aqiBarFill.style.width = `${100 - barWidth}%`;

    

    // Update breathing indicator

    const breathingCircle = aqiBreathingIndicator.querySelector('.breathing-circle');

    breathingCircle.style.background = `linear-gradient(135deg, ${aqiInfo.color}, ${aqiInfo.color}dd)`;

    breathingCircle.style.animationDuration = aqiInfo.breathingSpeed;

    breathingCircle.style.boxShadow = `0 0 20px ${aqiInfo.color}80`;
...
async function fetchAQIData(latitude, longitude) {
    ...
    aqiUrl.searchParams.set("current", "pm2_5,pm10");
    ...
    if (!response.ok) {
        ...
        // If AQI data is not available, return default values

        return { current: { pm2_5: 25, pm10: 35 } };
...
async function renderWeatherFromPosition(position, statusLabel = "Location updated.") {
    ...
    await renderCurrentWeather(placeName, weather, latitude, longitude);
...
async function loadWeatherByCity(city) {
    ...
    try {
        ...
        await renderCurrentWeather(cityData.name, weather, cityData.latitude, cityData.longitude);
